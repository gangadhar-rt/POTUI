...
@Component({
  selector: 'app-precontract-list',
  templateUrl: './precontract-list.component.html',
  styleUrls: ['./precontract-list.component.less'],
  providers: [ApiService]
})
export class PrecontractListComponent implements OnInit {
  records = 10;
  ...
  getLatestPreContracts(onload?) {
    ...
    if (onload) {
      ...
      request = {
        "status": 1, "loginUser": true, "fromDate": this.curr_date + "-" + this.m_names[this.prev_month - 1] + "-" + this.curr_year,
    ...
    this._service.PostService(request, '/procurement/getLatestPreContracts')
      .subscribe(
        data => {
          this.selectedlist = [];
