...
export class EmpWageComponent implements OnInit {
  ...
  saveDetails() {
    ...
    if (this.Isedit) {
      ...
      request = {
        'employeeWageRateTOs': [
          {
            'code': this.emp_wages_creation.value.code, 'name': this.emp_wages_creation.value.name,
            'wageFactor': this.emp_wages_creation.value.factor, 'status': 1, 'selected': false, 'duplicateFlag': false
          }
        ]
      };
    }
  ...
  deactive() {
    const req = { 'empWageIds': this.selectedlist.map((data) => data.wageRateId), 'status': 2 };
    this._service.PostService(req, '/centrallib/deleteEmpWages')
      .subscribe((data) => {
        this.selectedlist = [];
        this.getEWRFlassification();
      },
