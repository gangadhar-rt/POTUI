...
export class EmployeeClassificationComponent implements OnInit {
  ...
  saveDetails() {
    ...
    if (this.Isedit) {
      ...
      request = {
        'empClassTOs': [{
          'code': this.Employee_creation.value.code, 'name': this.Employee_creation.value.name,
          'status': 1, 'selected': false, 'duplicateFlag': false, 'measureUnitTO':
  ...
  deactive() {
    const req = { 'weatherIds': this.selectedlist.map((data) => data.id), 'status': 2 };
    this._service.PostService(req, '/centrallib/deleteEmpClasses')
      .subscribe((data) => {
        this.selectedlist = [];
        this.getEmployeeClassification();
      },
