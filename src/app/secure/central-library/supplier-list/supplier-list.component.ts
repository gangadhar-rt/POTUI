...
export class SupplierListComponent implements OnInit {
  ...
  viewRecord(record) {
    this.viewrecords = record.id;
    this.getCompanyDetails(record.id);
  ...
  selectedRecords(code, e) {
    ...
    if (code.checked) {
      ...
      const index = this.selectedlist.map(function (e) { return e.cmpCode; }).indexOf(code.cmpCode);
      this.selectedlist.splice(index, 1);
    }
    if (this.selectedlist.length === 1) {
      this.getCompanyDetails(this.viewrecords);
    } else {
  ...
  selectedaddressRecords(code, e) {
    ...
    if (code.checked) {
      ...
      const index = this.selectedAddress.map(function (e) { return e.addressId; }).indexOf(code.addressId);
  ...
  selectedcontactRecords(code, e) {
    ...
    if (code.checked) {
      ...
      const index = this.selectedContact.map(function (e) { return e.contactId; }).indexOf(code.contactId);
  ...
  selectedCurProjRecords(code, e) {
    ...
    if (code.checked) {
      ...
      const index = this.selectedContact.map(function (e) { return e.id; }).indexOf(code.id);
  ...
  editAddress() {
    this.address_creation = this.fb.group({
      ...
      'pincode': [this.selectedAddress[0].pincode, [Validators.required, FormsValidationService.numberOnly,
  ...
  editContact() {
    this.contact_creation = this.fb.group({
      ...
      'mobile': [this.selectedContact[0].mobile, [Validators.required, FormsValidationService.numberOnly,
      Validators.minLength(10), Validators.maxLength(10)]],
      'phone': [this.selectedContact[0].phone, [Validators.required, FormsValidationService.numberOnly,
      Validators.minLength(10), Validators.maxLength(10)]],
  ...
  editProject() {
    this.getProjects();
    const d = new Date(this.selectedCurproj[0].finishDate);
    const d1 = new Date(this.selectedCurproj[0].startDate);
    this.project_creation = this.fb.group({
      ...
      'startDate': [{
        ...
        formatted: this.selectedCurproj[0].startDate
      }, Validators.required],
      'finishDate': [{
        date: { year: d.getFullYear(), month: d.getMonth() + 1, day: d.getDate() },
        formatted: this.selectedCurproj[0].finishDate
      }, Validators.required],
