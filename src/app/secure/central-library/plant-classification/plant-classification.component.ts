...
export class PlantClassificationComponent implements OnInit {
  ...
  savePlant() {
    ...
    if (this.Isedit) {
      ...
      request = {
        'plantClassTOs': [
          {
            'code': this.plant_creation.value.plant_code, 'name': this.plant_creation.value.plant_code,
            'measureUnitTO.name': '', 'status': 1, 'selected': false, 'duplicateFlag': false,
  ...
  deactive() {
    const req = { 'plantClassIds': this.selectedlist.map((data) => data.id), 'status': 2 };
    this._service.PostService(req, '/centrallib/deletePlantClasses')
      .subscribe((data) => {
        this.selectedlist = [];
        this.getPlantClassification();
      },
