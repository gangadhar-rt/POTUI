...
export class ServiceClassificationComponent implements OnInit {
  ...
  saveDetails() {
    ...
    if (this.Isedit) {
      ...
      request = {
        'serviceClassTOs': [
          {
            'code': this.service_creation.value.code, 'name': this.service_creation.value.name,
            'status': 1, 'selected': false, 'duplicateFlag': false
          }
        ]
      };
    }
  ...
  deactive() {
    const req = { 'serviceClassIds': this.selectedlist.map((data) => data.id), 'status': 2 };
    this._service.PostService(req, '/centrallib/deleteServiceClasses')
      .subscribe((data) => {
        this.selectedlist = [];
        this.getServiceClassification();
      },
