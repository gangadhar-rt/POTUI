...
import { ConstBudgetComponent } from '../../schedule/const-budget/const-budget.component';
@Component({
...
export class AssetMaintenanceCategoryComponent implements OnInit {
  ...
  deactive() {
    const req = { 'assetIds': this.selectedlist.map((data) => data.id), 'status': 2 };
    this._service.PostService(req, '/centrallib/deleteAssetMaintenanceCategory')
      .subscribe((data) => {
        this.selectedlist = [];
        this.getList();
      },
