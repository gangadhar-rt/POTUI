...
export class WeatherClassificationComponent implements OnInit {
  ...
  saveDetails() {
    ...
    if (this.Isedit) {
      ...
      request = {
        'weatherTOs': [{
          'code': this.weather_creation.value.code,
          'name': this.weather_creation.value.name, 'status': 1, 'selected': false, 'duplicateFlag': false
        }]
      };
    }
  ...
  deactive() {
    ...
    this._service.PostService(req, '/centrallib/deleteWeathers')
      .subscribe((data) => {
        this.selectedlist = [];
        this.getWeatherClassification();
      },
